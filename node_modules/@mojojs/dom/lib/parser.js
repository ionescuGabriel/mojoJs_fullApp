import { BLOCK, CLOSE, EMPTY, END, NAME_START_CHAR, NAME_CHAR, NO_MORE_CONTENT, PHRASING, RAW, RCDATA, SCOPE } from './constants.js';
...
const TAG_ATTR_RE = new RegExp(`\\s*(${NAME_RE.source})(?:\\s*=\\s*(?:(?:"([^"]*)")|(?:'([^']*)')|([^>\\s]*)))?`, 'ysu');
    ...
    _end(current, xml, tag) {
        ...
        while (node !== null) {
            ...
            if (node.nodeType === '#element') {
                ...
                if (SCOPE.has(tagName) === true && tagName !== tag)
                    break;
                // Right tag
                if (tagName === tag) {
                    // "template"
                ...
                if (xml === false && PHRASING.has(tag) === true && PHRASING.has(tagName) === false)
                    break;
            }
            node = parent;
        }
        ...
        if (xml === false && current.nodeType === '#element') {
            ...
            // Close allowed parent elements in scope
                ...
                while (scope.has(parent.tagName) === false) {
                    if (allowed.has(parent.tagName) === true)
                        current = this._end(current, xml, parent.tagName);
                    if (parent.parentNode === null || parent.parentNode.nodeType !== '#element')
                        break;
