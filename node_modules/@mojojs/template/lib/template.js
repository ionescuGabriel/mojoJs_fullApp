...
export default class Template {
    ...
    _compileFn(code, escape, source) {
        if (DEBUG === true)
            process.stderr.write(`-- Template (${source.name})\n${code}`);
        try {
            // Compile to async function in a vm to get line numbers for syntax errors
            const script = new vm.Script(`async function __template (__locals, __source, __context, __escape, __safe) {${code}}; __template;`);
...
export function tmpl(strings, ...values) {
    let template = '';
...
function appendNodes(ast, ...nodes) {
...
function parseBlock(text, op) {
    ...
    if (blockMatch === null)
        return [{ op, value: text.replaceAll(BLOCK_REPLACE_RE, blockReplace) }];
    const node = { op: blockMatch[2] === '/' ? 'blockEnd' : 'blockStart', value: blockMatch[3] };
    if (blockMatch[4] !== '')
        node.hints = blockMatch[4];
    return [...parseBlock(blockMatch[1].trimEnd(), op), node, ...parseBlock(blockMatch[5], op)];
}
function parseLine(line, op, isLastLine) {
    const nodes = [];
    let trim = false;
    ...
    while (length > sticky.offset) {
        ...
        // Tag start
            ...
            if (startMatch !== null) {
                ...
                if (leftovers !== '')
                    appendNodes(nodes, ...parseBlock(leftovers, op));
                // Replacement
                if (type === '%') {
    ...
    if (nodes.length > 0) {
        ...
        if (lastOp === 'blockStart' || lastOp === 'blockEnd')
            trim = true;
    }
    // Newline
    if (op === 'text' && trim === false && isLastLine === false)
        appendNodes(nodes, { op, value: '\n' });
...
function throwWithContext(error, source) {
