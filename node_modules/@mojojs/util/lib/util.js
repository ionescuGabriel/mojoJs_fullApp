...
export function headerParams(value) {
    ...
    while (value.length > sticky.offset) {
        ...
        if (quotedMatch !== null) {
            params[name] ?? (params[name] = headerUnquote(quotedMatch[1]));
            continue;
        }
        const unquotedMatch = stickyMatch(sticky, UNQUOTED_RE);
        if (unquotedMatch !== null)
            params[name] ?? (params[name] = unquotedMatch[1]);
    }
 ...
 */
export function headerQuote(value) {
    return '"' + value.replaceAll('\\', '\\\\').replaceAll('"', '\\"') + '"';
}
/**
 * Unquote HTTP header field parameter.
 */
export function headerUnquote(value) {
