...
export class Server {
    ...
    async stop() {
        ...
        for (const url of this.urls) {
            if (url.protocol === 'http+unix')
                await new Path(urlToSocketPath(url)).rm();
        }
        const { app } = this;
    ...
    _handleRequest(req, raw) {
        const { app } = this;
        const { socket } = req;
        const ctx = app.newContext(this._prepareRequest(req, socket, false), new ServerResponse((res, body) => {
    ...
    _handleUpgrade(wss, req, socket, head) {
        const { app } = this;
        const ctx = app.newContext(this._prepareRequest(req, socket, true), new ServerResponse((res, body) => {
...
function sendResponse(res, body, raw) {
