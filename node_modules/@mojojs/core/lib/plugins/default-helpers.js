...
export default function defaultHelpersPlugin(app) {
    ...
    for (const type of fields) {
        app.addHelper(`tags.${type}Field`, async (ctx, name, attrs = {}) => {
    ...
    for (const type of fields) {
        app.addHelper(`${type}FieldTag`, async (ctx, name, attrs = {}) => {
            return await inputTag(ctx, name, { ...attrs, type });
        });
    }
...
function assetTag(ctx, path, attrs) {
    const url = ctx.urlForAsset(path);
    if (/\.js$/.test(url))
        return ctx.tag('script', { src: url, ...attrs });
    if (/\.css$/.test(url))
        return ctx.tag('link', { rel: 'stylesheet', href: url, ...attrs });
    return ctx.tag('img', { src: url, ...attrs });
}
function buttonTo(ctx, target, attrs, text) {
    return ctx.formFor(target, attrs, ctx.submitButtonTag(text));
}
async function checkBoxTag(ctx, name, attrs = {}) {
...
function faviconTag(ctx, file) {
    return ctx.tag('link', { rel: 'icon', href: ctx.urlForFile(file ?? '/mojo/favicon.ico') });
}
async function fileFieldTag(ctx, name, attrs = {}) {
    return tag(ctx, 'input', { ...attrs, type: 'file', name });
}
function formFor(ctx, target, attrs, content) {
...
async function hiddenFieldTag(ctx, name, value, attrs = {}) {
...
function imageTag(ctx, target, attrs = {}) {
    return ctx.tag('img', { src: ctx.urlForFile(target), ...attrs });
}
async function inputTag(ctx, name, attrs = {}) {
    attrs.name = name;
...
async function labelFor(ctx, name, value, attrs = {}) {
...
async function passwordFieldTag(ctx, name, attrs = {}) {
    return tag(ctx, 'input', { ...attrs, type: 'password', name });
}
async function proxyGet(ctx, url, config) {
    await ctx.proxyRequest({ method: 'GET', url, ...config });
}
async function proxyPost(ctx, url, config) {
    await ctx.proxyRequest({ method: 'POST', url, ...config });
}
async function proxyRequest(ctx, config) {
...
function tag(ctx, name, attrs = {}, content = '') {
    ...
    if (content instanceof Promise)
        return content.then(text => new SafeString(DOM.newTag(name, attrs, text).toString()));
    // Temporary workaround for backwards compatibility, to be removed in 2.0
    return new SafeString(DOM.newTag(name, attrs, content).toString());
}
async function textAreaTag(ctx, name, attrs = {}, content) {
    attrs.name = name;
    const params = await ctx.params();
